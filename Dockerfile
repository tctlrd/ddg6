...
RUN sed -i 's/#log_path = syslog/log_path = \/var\/log\/dovecot\/dovecot.log/' /etc/dovecot/conf.d/10-logging.conf \
 ...
 && sed -i 's/#\?ssl_cert =.*/ssl_cert = <\/etc\/ssl\/private\/tls.crt/' /etc/dovecot/conf.d/10-ssl.conf \
 && sed -i 's/#\?ssl_key =.*/ssl_key = <\/etc\/ssl\/private\/tls.key/' /etc/dovecot/conf.d/10-ssl.conf \
    # mailboxes
